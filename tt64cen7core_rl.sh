...
clean_proxies() {
    eecho "Cleaning previous proxies"
    /usr/bin/pkill 3proxy
    /bin/rm -f /usr/local/3proxy/installer/boot_*
    /bin/rm -f /usr/local/3proxy/installer/data.txt
    /bin/rm -f /usr/local/3proxy/proxy.txt
    /bin/rm -f /etc/rc.local
    eecho "Done cleaning previous proxies"
}

clean_iptables() {
    eecho "Cleaning previous iptables rules"
    iptables-save | grep -- '-A INPUT -p tcp -m tcp -m state --state NEW -j ACCEPT' | sed 's/-A/-D/' | iptables-restore
    eecho "Done cleaning iptables rules"
}

clean_ifconfig() {
    eecho "Cleaning previous IPv6 addresses"
    /sbin/ip -6 addr show dev $ETHNAME | awk '/inet6 / {print $2}' | sed 's/\/.*//' | while read ip; do
        /sbin/ip -6 addr del ${ip}/${IP6PREFIXLEN} dev $ETHNAME
    done
    eecho "Done cleaning IPv6 addresses"
}
